...
多台机器一起训练时，跨机通信又慢又贵。纯 FSDP 把参数切到**所有卡**（包括
别的机器），每层前向都要跨机 gather 参数。HSDP 的办法：**机器内分片**
（省显存，通信走机内快车道）、**机器间复制**（每台机器各持完整副本，跨机
只同步梯度，而且只传 1/N 的量）。

```
纯 FSDP（8 卡全分片）:
  每层前向 gather 8 份参数，其中 4 份跨机 —— 贵

HSDP（机器内分片 + 机器间复制）:
  机器 A：4 卡之间分片，组内 reduce-scatter 梯度（机内，快）
  机器 B：同样
  机器间：只 all-reduce 一次"分片梯度"（跨机只传 1/4 的量）
```

即"部门内分片、部门间复制"：分片组（shard group）= 机器内的卡，
复制组（replicate group）= 不同机器上同一位置的卡。
