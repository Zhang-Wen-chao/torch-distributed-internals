...
流水线两端有空转（"气泡"），气泡比例 = (段数-1)/(段数-1+micro-batch 数)，
**和调度方式无关**。1F1B 调度（一次前向一次反向交替）解决的**不是气泡**，
而是两件更要命的事：**激活显存**（任意时刻只需存最多"段数-1"个 batch 的
激活，而不是全部 batch——这就是为什么 micro-batch 要切得很细）和**反向
提前开始**（不用等所有前向跑完）。官方：
